...
describe Card do
  ...
  context "update review date" do
    ...
    it "on 12 hour after first check" do
      expect { card.check_translation(card.original_text) }.to change { card.review_date.beginning_of_hour }.by( 12.hours )
    end

    it "on 3 days after second check" do
      card.update_attribute(:box, 1)
      expect { card.check_translation(card.original_text) }.to change { card.review_date.beginning_of_day }.by(3.days)
    end

    it "on 1 week after third check" do
      card.update_attribute(:box, 2)
      expect { card.check_translation(card.original_text) }.to change { card.review_date.beginning_of_week }.by(1.week)
    end

    it "on 2 week after forth check" do
      card.update_attribute(:box, 3)
      expect { card.check_translation(card.original_text) }.to change { card.review_date.beginning_of_week }.by(2.week)
    end

    it "on a month after fifth check" do
      card.update_attribute(:box, 4)
      expect { card.check_translation(card.original_text) }.to change { card.review_date.beginning_of_month }.by(1.months)
    end
    it "on a 12 hours from high level if three errors occurs" do
      card.update_attributes(box: 4)
