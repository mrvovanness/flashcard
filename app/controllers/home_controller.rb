class HomeController < ApplicationController
  ...
  def check_card
    ...
    respond_to do |format|
      ...
      when 5 then
        @response_alert = t('card.check_success')
      when 4, 3 then
        @response_alert = t('card.check_delay')
      when 2 then
        @response_alert = t('card.typos',
                            correct_answer: card.original_text,
                            typed_word: check_params[:user_translation])
      else
        @response_alert = t('card.check_error', right_card: card.original_text)
      end
      format.json
    end
  end
